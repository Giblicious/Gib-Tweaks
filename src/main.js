...
/* ─── Default Settings ────────────────────────────────────────────────
   Empty string '' or 0 = use theme/Obsidian default (no override). */
...
const DEFAULT_SETTINGS = {
  ...
  // ── Per-heading ──
  h1Size: '', h1Weight: 0, h1LineHeight: '', h1LetterSpacing: '',
  h2Size: '', h2Weight: 0, h2LineHeight: '',
  h3Size: '', h3Weight: 0, h3LineHeight: '',
  h4Size: '', h4Weight: 0,
  h5Size: '', h5Weight: 0,
  h6Size: '', h6Weight: 0,
...
function buildCSS(s) {
  const lines = [];
  const add = (name, val) => {
    if (val !== '' && val !== 0 && val !== undefined) lines.push(`  ${name}: ${val};`);
  };
...
/* ─── Helpers for native Obsidian setting groups ──────────────────── */
...
class TypographySettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Body text',
        'Base text properties for paragraphs and reading view.');
      addTextSetting(items, p, 'Font size', 'bodySize',
        'Base size for note and chat body text', '16px');
      addTextSetting(items, p, 'Line height', 'lineHeight',
        'Spacing between lines', '1.5');
      addWeightSetting(items, p, 'Paragraph weight', 'paragraphWeight',
        'Weight for paragraphs and list items');
    }

    // ════════════════════════════════════════════════════════════════
    //  HEADINGS — shared color
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Heading color',
        'Shared color across all heading levels, inline title, and chat headings.');
      addTextSetting(items, p, 'Color', 'headingColor', '', '#D4C5B5');
    }

    // ── Per-level heading groups ──
    const levels = [
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Inline title',
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Code',
        'Code blocks, inline code, and terminal output across notes and Better Claude.');
      addTextSetting(items, p, 'Font size', 'codeSize',
        'Size for code blocks and inline code', '');
      addWeightSetting(items, p, 'Weight', 'codeWeight',
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Emphasis',
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'Workspace',
      ...
      new Setting(items)
        ...
        .setDesc(names.length
          ? 'Loaded automatically when Obsidian starts.'
          : 'Enable the core Workspaces plugin and save a workspace first.')
        .addDropdown(drop => {
          drop.addOption('', 'None');
      ...
      new Setting(items)
        .setName('Hide sidebar panels on startup')
        .setDesc('Closes Backlinks, Outgoing links, Tags, and Properties panels that Obsidian re-opens on every reload.')
        .addToggle(toggle => {
    ...
    // ════════════════════════════════════════════════════════════════
    {
      const items = createSettingGroup(containerEl, 'UI chrome',
        'Obsidian interface text: sidebar labels, tab titles, menus, settings, toolbar buttons.');
      addTextSetting(items, p, 'UI small text', 'uiSmallSize',
        'Primary UI text (sidebar items, menu items, setting labels)', '13px');
      addTextSetting(items, p, 'UI smaller text', 'uiSmallerSize',
        'Secondary UI text (badges, status indicators, footnotes)', '11px');
    }
...
module.exports = class GibTweaksPlugin extends Plugin {
  async onload() {
    ...
    // Status bar positioning + workspace tweaks
    this.app.workspace.onLayoutReady(() => {
      ...
      // Close unwanted sidebar panels that Obsidian re-creates on every reload
